...
from database import update_entities,save_entity, save_message

# set_debug(True)

class AICompanionAgent:
    def __init__(self, prompt_template, messages_history_threshold: int = 15, verbose: bool = True):
...
class EntitiesExtractionAgent:
    def __init__(self, tools: List, is_agent_verbose: bool = True, max_iterations: int = 3, return_thought_process: bool =True):
    ...
    def update_user_profile(self, user_input):
        ...
        user_message_id = save_message(user_input, is_user=True)
        
        result = self.agent({"input": user_input})

        ai_message_id = save_message(result['output'], is_user=False)

        update_entities(entities, user_message_id)
